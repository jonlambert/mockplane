 ...
 */
test('client-side: page.route() intercepts browser fetch', async ({ page }, testInfo) => {
  await createMockNetwork({ page, testInfo })
    .handle({
      url: 'https://jsonplaceholder.typicode.com/todos?_limit=3',
      request: { method: 'GET' },
      response: { status: 200, body: todos },
    })
    .commit();
 ...
 */
test('server-side: MSW intercepts createServerFn fetch', async ({ page }, testInfo) => {
  const posts = [
